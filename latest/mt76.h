...
int mt76_get_antenna(struct ieee80211_hw *hw, u32 *tx_ant,
		     u32 *rx_ant);
...
int mt76_npu_net_setup_tc(struct ieee80211_hw *hw, struct ieee80211_vif *vif,
			  ...
			  void *type_data);
#else
/* Stub DMA descriptor types -- the real definitions live in
 * airoha_offload.h which is only present when CONFIG_MT76_NPU is enabled.
 * These stubs match the size of mt76_desc so the sizeof() calls in dma.c
 * compile; the NPU queue paths are never taken without CONFIG_MT76_NPU.
 */
